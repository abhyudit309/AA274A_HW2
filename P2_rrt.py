...
class RRT(object):
    ...
    def shortcut_path(self):
        ...
        success = False
        self.path = np.array(self.path).tolist()  # converts it to a list of lists from a list of numpy arrays
        while not success:
            success = True
            for i in range(len(self.path)):
                if (self.path[i] != self.x_init).any() and (self.path[i] != self.x_goal).any():
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        shortest_lengths = [dubins.shortest_path(state, x, self.turning_radius).path_length() for state in V]
        return np.argmin(shortest_lengths)
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        steering_distance = dubins.shortest_path(x1, x2, self.turning_radius).path_length()
        if steering_distance < eps:
            return x2
        else:
            no_of_samples = 500
            step_size = steering_distance / no_of_samples
            qs, ts = dubins.shortest_path(x1, x2, 1.001 * self.turning_radius).sample_many(step_size)
            dist_closest_to_eps = max([dist for dist in ts if dist <= eps])
            return qs[ts.index(dist_closest_to_eps)]
